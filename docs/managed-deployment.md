...
`urlString` must be an SMB URL with a server and share path. Usernames and passwords in the URL cause the managed configuration to be rejected. A VPN rule is valid only when `vpnRuleEnabled` is `true` and `vpnName` names a specific VPN visible in System Settings.
